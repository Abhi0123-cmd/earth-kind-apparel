...
function AdminReplacementsContent() {
  ...
  const { data: tickets, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .ilike("subject", "%Replacement Request%")
  ...
  const { data: messages } = useQuery({
    queryKey: ["admin-replacement-messages", selectedTicket],
    queryFn: async () => {
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ id, status, oldStatus }: { id: string; status: TicketStatus; oldStatus: string }) => {
      ...
      if (oldStatus !== status) {
        await logActivity("Replacement status changed", "ticket", id, { from: oldStatus, to: status });
      }
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["admin-replacements"] }),
  });
  ...
  const sendReply = useMutation({
    mutationFn: async () => {
      ...
      if (error) throw error;
      await logActivity("Admin replied to replacement", "ticket", selectedTicket, {});
    },
    onSuccess: () => {
      setReply("");
      queryClient.invalidateQueries({ queryKey: ["admin-replacement-messages", selectedTicket] });
    },
  });
  ...
  return (
    <div>
      <h1 className="font-display text-4xl mb-6">REPLACEMENTS</h1>
      <p className="text-sm text-muted-foreground font-body mb-6">
        Replacement requests submitted by customers. Select a request to view details and reply.
      </p>

      <div className="flex flex-wrap gap-3 mb-6">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search replacements..."
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-2 space-y-2 max-h-[70vh] overflow-y-auto">
          {filtered.length === 0 ? (
            <p className="text-muted-foreground font-body">No replacement requests found.</p>
          ) : (
        ...
        <div className="lg:col-span-3">
          {selectedTicket ? (
            ...
            <div className="border border-border p-12 text-center">
              <p className="text-muted-foreground font-body">Select a replacement request to view details</p>
            </div>
          )}
        </div>
      </div>
