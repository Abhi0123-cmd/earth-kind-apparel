...
function AdminSupportContent() {
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ id, status, oldStatus }: { id: string; status: TicketStatus; oldStatus: string }) => {
      ...
      if (oldStatus !== status) {
        await logActivity("Ticket status changed", "ticket", id, { from: oldStatus, to: status });
      }
    },
  ...
  const sendReply = useMutation({
    mutationFn: async () => {
      ...
      if (error) throw error;
      await logActivity("Admin replied to ticket", "ticket", selectedTicket, {});
    },
    onSuccess: () => {
      setReply("");
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-2 max-h-[70vh] overflow-y-auto">
          {filtered.length === 0 ? (
            <p className="text-muted-foreground font-body">No tickets found.</p>
          ) : (
